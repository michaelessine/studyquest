...
export async function POST(req: NextRequest) {
  ...
  try {
    const response = await anthropic.messages.create({
      ...
      max_tokens: 4096,
      system: 'Extract every course from this transcript. Return ONLY a JSON array — no preamble — where each item has: courseName, courseCode, grade (as a float, e.g. 3.5), year, semester. If a field is missing, use null.',
      messages: [{
        role: 'user',
        content: [
          {
