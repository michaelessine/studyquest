...
export async function POST(req: NextRequest) {
  const { exerciseSetId, skillNodeIds, pctSolved } = await req.json()
  ...
  if (exerciseSetId) {
    await prisma.exerciseSet.update({
      ...
      data: { masteryImpact: masteryImpact as Prisma.InputJsonValue },
    }).catch(() => {})
  }

  return NextResponse.json({ applied, gainPerTopic: delta })
}
